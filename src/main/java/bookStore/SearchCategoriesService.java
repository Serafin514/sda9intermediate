...
public class SearchCategoriesService {
    ...
    public List<AdminCategoryDTO> filterCategories(String searchText) {
        return source.getCategories().stream()
                .map(category -> buildAdminCategoryDTO(category))
                .peek(e->
                        e.setParent(findParent(e)
                                .map(m->buildAdminCategoryDTO(m)).orElse(null)))
                .peek(e -> {
                    if (e.getText().equals(searchText)) {
    ...
    private Optional<Category> findParent(AdminCategoryDTO child) {
        ...
        return source.findCategoryById(Integer
    ...
    private AdminCategoryDTO buildAdminCategoryDTO(Category category) {
        return AdminCategoryDTO.builder()
                ...
                .parentCategoryId(category.getParentsId().toString())
